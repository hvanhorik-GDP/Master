...
cPhysics_Project_01_Tilter* gPhysics_Project_01_Tilter = NULL;


void Physics_Project_01_key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	// Ignore key up (double hit)
	if (action == 0)
		return;

	if (!gPhysics_Project_01_Tilter)
	...
	if (!isShiftKeyDownByAlone(mods) && !isCtrlKeyDownByAlone(mods))
	{
		...
		if (key == GLFW_KEY_T)
		{
			gPhysics_Project_01_Tilter->track();			// Turn on ball tracking
		}
		...
		{
			gPhysics_Project_01_Tilter->tiltPlane(z, planeTilt);
		}
		...
		if (key == GLFW_KEY_UP)
		{
			gPhysics_Project_01_Tilter->tiltPlane(x, planeTilt);
		}
...
void Physics_Project_01_mouse_button_callback(GLFWwindow* window, int button, int action, int mods)
{

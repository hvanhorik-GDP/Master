...
static bool gDay = true;
static float g_Saved_constantAttenuation;
static float g_Saved_linearAttenuation;
static float g_Saved_quadradicAttenuation;

void Graphics_MidTerm_2019_ToObject::SwitchSun()
{
	cObjectManager objectManager;

	auto object = objectManager.FindObjectByName("light_0");
	assert(object);
	cObject_Light* light = dynamic_cast<cObject_Light*>(object);
	assert(light);

	if (gDay)
	{
		g_Saved_constantAttenuation = light->constantAttenuation;
		g_Saved_linearAttenuation = light->linearAttenuation;
		g_Saved_quadradicAttenuation = light->quadradicAttenuation;
		light->constantAttenuation *= 1.95;
		light->linearAttenuation *= 1.95;
		light->quadradicAttenuation *= 1.95;
	}
	else
	{
		light->constantAttenuation = g_Saved_constantAttenuation;
		light->linearAttenuation = g_Saved_linearAttenuation;
		light->quadradicAttenuation = g_Saved_quadradicAttenuation;
	}
	gDay = !gDay;
}

...
void cAudioObjectManager::LoadObjects(rapidxml::xml_node<>* parent, cAssetManager* objectManager)
{
	...
	for (size_t i = 0; i < groups.GetSize(); ++i)
	{
		...
		if (group.isValid())
		{
			...
			else
			{
				...
				//			"models"
				//			"fonts"
				std::cout << "cAudioObjectManager - Ignoring: " << type.GetValue() << std::endl;
			}
		}
	}
}

std::ostream& operator<<(std::ostream& stream, const cAudioObjectManager& val)
{
	return stream;
}

void cAudioObjectManager::PlaySomething()
{

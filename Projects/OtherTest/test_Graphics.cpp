...
int test_Graphics(gamelibrary::GameLibrary& gameLib, cObject_World_Init * init)
{
	...
	glfwSetKeyCallback(window, world->keyCallback);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		projection = glm::perspective(1.0f,						// FOV
							 ratio,								// Aspect ratio
							 world->cameraNearClipping,			// Near clipping plane
							 world->cameraFarClipping);			// Far clipping plane

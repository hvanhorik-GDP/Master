...
#include "ObjectItems/cObject_Camera.h"
...
cObjectManager_Camera::cObjectManager_Camera()
{
}
...
void cObjectManager_Camera::LoadObjects(rapidxml::xml_node<>* node)
{
	...
	auto object = new cObject_Camera(
		objectType.GetValue(),
		objectName.GetValue(),
...
void cObjectManager_Camera::SaveObject(iObject* inObject, rapidxml::xml_node<>* parent)
{
	assert(parent);
	assert(inObject);
	auto object = dynamic_cast<cObject_Camera*>(inObject);
	assert(object);
	...
	if (m_map_objects.find(object->GetName()) != m_map_objects.end())
	{
		...
		writeObject = dynamic_cast<cObject_Camera*>(m_map_objects[object->GetName()]);
	}
	else
...
iObjectManager::iObject_map* cObjectManager_Camera::GetObjects(const std::string& name)
{
	return &m_map_objects;
}

// For debugging purposes - dumps the contents in human readable form
std::ostream& operator<<(std::ostream& stream, const cObjectManager_Camera& val)
{
	return stream;
}


void cObjectManager_Camera::ResolveAlias()
{

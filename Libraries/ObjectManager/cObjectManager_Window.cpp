...
#include "ObjectItems/cObject_Window.h"
...
cObjectManager_Window::cObjectManager_Window()
{
}
...
void cObjectManager_Window::LoadObjects(rapidxml::xml_node<>* node)
{
	...
	auto object = new cObject_Window(
		objectType.GetValue(),
		objectName.GetValue(),
...
void cObjectManager_Window::SaveObject(iObject* inObject, rapidxml::xml_node<>* parent)
{
	assert(parent);
	assert(inObject);
	auto object = dynamic_cast<cObject_Window*>(inObject);
	assert(object);
	...
	if (m_map_objects.find(object->GetName()) != m_map_objects.end())
	{
		...
		writeObject = dynamic_cast<cObject_Window*>(m_map_objects[object->GetName()]);
	}
	else
...
iObjectManager::iObject_map* cObjectManager_Window::GetObjects(const std::string& name)
{
	return &m_map_objects;
}

// For debugging purposes - dumps the contents in human readable form
std::ostream& operator<<(std::ostream& stream, const cObjectManager_Window& val)
{
	return stream;
}


void cObjectManager_Window::ResolveAlias()
{
